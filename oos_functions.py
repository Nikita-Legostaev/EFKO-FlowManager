...
def _parse_month_series(series: pl.Series) -> pl.Series:
    return pl.Series(series.name,
                     [_parse_ru_month(v) for v in series.to_list()],
                     dtype=pl.Date)
    ...
    for col in df.columns:
        ...
        try:
            if hasattr(s.dtype, "numpy_dtype"):
                result[name] = pl.Series(name, s.to_numpy(dtype=s.dtype.numpy_dtype, na_value=None))
            ...
            try:
                ...
                result[name] = pl.Series(name, [None if (v != v) else v for v in s.tolist()])
...
def _pl2pd(df: pl.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        s = df[col]
        if s.dtype == pl.Date:
            result[col] = [v.isoformat() if v is not None else None for v in s.to_list()]
        else:
            result[col] = s.to_list()
    return pd.DataFrame(result)

# ─── Чтение листов ────────────────────────────────────────────────────────
def _read_sheet(filepath: Path, sheet: str, engine: str, header_row: int) -> pl.DataFrame:
...
def _query_emkost(kub: Path) -> pl.DataFrame:
    ...
    df = df.with_columns([
        pl.col("Сеть UNI").cast(pl.Utf8), pl.col("Формат").cast(pl.Utf8),
    ...
    return df.select(["Сеть UNI","Формат","сеть&формат","Год","Месяц",
                      "Ф_ТТ присутствия","Емкость сети, тн."])
...
def _query_elt_status(elt: Path, sheet: str) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        pl.when(pl.col("Итог").cast(pl.Float64) == 1.0)
          .then(pl.lit("не берем")).otherwise(pl.lit("берем")).alias("Статус"),
    ])
    g = df.group_by(["Адрес","Месяц","Статус"]).agg(pl.len().alias("Количество"))
    piv = g.pivot(values="Количество", index=["Адрес","Месяц"],
                  on="Статус", aggregate_function="sum")
    ...
    return piv.with_columns(
        ...
        ).then(pl.lit("берем")).otherwise(pl.lit("не берем")).alias("Статус ТТ(1)")
...
def _query_schet_tt(elt: Path, sheet: str) -> pl.DataFrame:
    ...
    df = df.with_columns(_parse_month_series(df["Месяц"]))
    df = df.with_columns(
        (pl.col("Торговая сеть").cast(pl.Utf8) + pl.col("Формат").cast(pl.Utf8)).alias("Сеть&формат")
    )
...
def _query_kub(kub: Path, sheet: str, header_row: int,
               elt_status: pl.DataFrame, asst: pl.DataFrame) -> pl.DataFrame:
    df = _read_kub(kub, sheet, header_row)
    df = df.with_columns(_parse_month_series(df["Месяц"]))
    df = df.with_columns([
        pl.col("Сеть UNI").cast(pl.Utf8), pl.col("Формат").cast(pl.Utf8),
        pl.col("Адрес").cast(pl.Utf8),
        (pl.col("Сеть UNI").cast(pl.Utf8) + pl.col("Формат").cast(pl.Utf8)).alias("Сеть&формат"),
    ])
    ...
    if "Ф_SKUср факт" in df.columns:
        df = df.with_columns(
            pl.col("Ф_SKUср факт").cast(pl.Float64, strict=False).round(0).cast(pl.Int64, strict=False)
        )
    df = df.join(elt_status.select(["Адрес","Месяц","Статус ТТ(1)"]),
                 on=["Адрес","Месяц"], how="left")
    df = df.filter(pl.col("Статус ТТ(1)") == "берем")
    asst_cols = ["Адрес","Месяц"] + [c for c in ("Ф_ SKUcрТТприс","Емкость, кг.,л.") if c in asst.columns]
    return df.join(asst.select(asst_cols), on=["Адрес","Месяц"], how="inner")

def _query_itogo(kub: Path, sheet: str,
                 emkost: pl.DataFrame, schet_tt: pl.DataFrame,
                 filter_type: str, formats: set) -> pl.DataFrame:
    ...
    df = df.select(["Сеть UNI","Формат","Сеть&формат","Год","Месяц",
                    "Ф_Ин-ть","Ф_ТТ присутствия","Продажи кг; л"])
    df = df.join(
        emkost.rename({"сеть&формат":"Сеть&формат"}).select(["Сеть&формат","Месяц","Емкость сети, тн."]),
        on=["Сеть&формат","Месяц"], how="left",
    )
    df = df.join(
...
def _query_competitor(kub: Path, sheet: str, header_row: int,
                      elt_status: pl.DataFrame) -> pl.DataFrame:
    df = _read_kub(kub, sheet, header_row)
    df = df.with_columns(_parse_month_series(df["Месяц"]))
    df = df.with_columns([
        pl.col("Сеть UNI").cast(pl.Utf8), pl.col("Формат").cast(pl.Utf8),
    ...
    if "Ф Цена продажи с НДС, руб/шт" in df.columns:
        df = df.with_columns(pl.col("Ф Цена продажи с НДС, руб/шт").cast(pl.Float64, strict=False))
    df = df.join(elt_status.select(["Адрес","Месяц","Статус ТТ(1)"]),
                 on=["Адрес","Месяц"], how="left")
    df = df.filter(pl.col("Статус ТТ(1)") == "берем")
...
def _save_to_excel(report_file: Path, sheets_dict: dict, log):
    ...
    for sheet_name, pl_df in sheets_dict.items():
        pdf = _pl2pd(pl_df)
        pos = wb.sheetnames.index(sheet_name) if sheet_name in wb.sheetnames else len(wb.sheetnames)
...
def run_oos_report(report_type: str, kub_file: str, elt_file: str,
                   report_file: str, log, stop_event):
    """
    ...
    asst   = _query_asst(kub);   chk()
    emkost = _query_emkost(kub); chk()
    log(f"  асс-т: {len(asst)} строк, емкость: {len(emkost)} строк")

    # ── Слобода ──
    if report_type == "sloboda":
        log("📋 Слобода: ELT выгрузки...")
        e200 = _query_elt_status(elt, "200"); chk()
        e400 = _query_elt_status(elt, "400"); chk()
        e800 = _query_elt_status(elt, "800"); chk()
        s200 = _query_schet_tt(elt, "200");   chk()
        s400 = _query_schet_tt(elt, "400");   chk()
        s800 = _query_schet_tt(elt, "800");   chk()
        log("📋 Слобода: КУБ + итого сети...")
        k200 = _query_kub(kub, "200", 9, e200, asst); chk()
        k400 = _query_kub(kub, "400", 9, e400, asst); chk()
        k800 = _query_kub(kub, "800", 9, e800, asst); chk()
        i200 = _query_itogo(kub,"итого сеть 200",emkost,s200,"include",_FORMATS_4); chk()
        i400 = _query_itogo(kub,"итого сеть 400",emkost,s400,"include",_FORMATS_4); chk()
        i800 = _query_itogo(kub,"итого сеть 800",emkost,s800,"include",_FORMATS_4); chk()
        sheets = {"200":k200,"400":k400,"800":k800,
                  "итого сеть 200":i200,"итого сеть 400":i400,"итого сеть 800":i800}

    # ── Провансаль ──
    elif report_type == "provansale":
        log("📋 Провансаль: ELT выгрузки...")
        e200 = _query_elt_status(elt, "провансаль 200"); chk()
        e400 = _query_elt_status(elt, "провансаль 400"); chk()
        e800 = _query_elt_status(elt, "провансаль 800"); chk()
        s200 = _query_schet_tt(elt, "провансаль 200");   chk()
        s400 = _query_schet_tt(elt, "провансаль 400");   chk()
        s800 = _query_schet_tt(elt, "провансаль 800");   chk()
        log("📋 Провансаль: КУБ + итого сети...")
        k200 = _query_kub(kub,"провансаль 200",8,e200,asst); chk()
        k400 = _query_kub(kub,"провансаль 400",8,e400,asst); chk()
        k800 = _query_kub(kub,"провансаль 800",8,e800,asst); chk()
        i200 = _query_itogo(kub,"итого сеть П200",emkost,s200,"exclude",_FORMATS_EX_P200); chk()
        i400 = _query_itogo(kub,"итого сеть П400",emkost,s400,"include",_FORMATS_4); chk()
        i800 = _query_itogo(kub,"итого сеть П800",emkost,s800,"include",_FORMATS_4); chk()
        log("📋 Конкуренты...")
        maheev   = _query_competitor(kub,"Махеев",  8,e400); chk()
        mr_ricco = _query_competitor(kub,"Mr Ricco",9,e400); chk()
        ryaba    = _query_competitor(kub,"Ряба",    9,e400); chk()
        yalg     = _query_competitor(kub,"ЯЛГ",     9,e400); chk()
        sheets = {"200":k200,"400":k400,"800":k800,
                  "итого сеть 200":i200,"итого сеть 400":i400,"итого сеть 800":i800,
                  "Махеев":maheev,"Mr.Ricco":mr_ricco,"Ряба":ryaba,"ЯЛГ":yalg}

    # ── Оливковый ──
    elif report_type == "olive":
        log("📋 Оливковый: ELT выгрузки...")
        e200 = _query_elt_status(elt, "оливковый 200"); chk()
        e400 = _query_elt_status(elt, "оливковый 400"); chk()
        e800 = _query_elt_status(elt, "оливковый 800"); chk()
        s200 = _query_schet_tt(elt, "оливковый 200");   chk()
        s400 = _query_schet_tt(elt, "оливковый 400");   chk()
        s800 = _query_schet_tt(elt, "оливковый 800");   chk()
        log("📋 Оливковый: КУБ + итого сети...")
        k200 = _query_kub(kub,"оливковый 200",8,e200,asst); chk()
        k400 = _query_kub(kub,"оливковый 400",8,e400,asst); chk()
        k800 = _query_kub(kub,"оливковый 800",8,e800,asst); chk()
        i200 = _query_itogo(kub,"итого сеть О200",emkost,s200,"include",_FORMATS_4); chk()
        i400 = _query_itogo(kub,"итого сеть О400",emkost,s400,"include",_FORMATS_4); chk()
        i800 = _query_itogo(kub,"итого сеть О800",emkost,s800,"include",_FORMATS_4); chk()
        sheets = {"200":k200,"400":k400,"800":k800,
                  "итого сеть 200":i200,"итого сеть 400":i400,"итого сеть 800":i800}
    else:
