...
def find_svod_file(folder, month_num):
    ...
    for fname in os.listdir(folder):
        if re.match(rf'^{month_str}[_\s]СВОД', fname, re.IGNORECASE) and fname.endswith('.xlsx'):
...
def extract_month_data(filepath, sheet_index=0):
    ...
    for col_offset, (company, group) in enumerate(zip(companies, groups)):
        ...
        if maslo_total != 0 or maslo_soy != 0 or maslo_rape != 0:
            if maslo_other != 0:
                results.append([group, company, 'Производство фасованного масла', 'Масло', maslo_other])
            if maslo_soy != 0:
                results.append([group, company, 'Производство фасованного масла', 'Соевое', maslo_soy])
            if maslo_rape != 0:
                results.append([group, company, 'Производство фасованного масла', 'Рапсовое', maslo_rape])
        ...
        if marg_total  != 0: results.append([group, company, 'Производство маргариновой продукции', 'Всего',               marg_total])
        if marg_mono   != 0: results.append([group, company, 'Производство маргариновой продукции', 'Монолит',             marg_mono])
        if marg_fas    != 0: results.append([group, company, 'Производство маргариновой продукции', 'Фасовка без спредов', marg_fas])
        if marg_spread != 0: results.append([group, company, 'Производство маргариновой продукции', 'Спреды/крем',         marg_spread])
        ...
        if mayo  != 0: results.append([group, company, 'Производство майонезов и майонезных соусов', 'Майонез', mayo])
        if sauce != 0: results.append([group, company, 'Производство майонезов и майонезных соусов', 'Соус',    sauce])
...
def load_mapping(mapping_path):
    ...
    for _, row in df.iterrows():
        if pd.notna(row['завод']) and pd.notna(row['строка']) and str(row['строка']).strip():
            result.append({
                'завод':    str(row['завод']).strip(),
                'подкат':   str(row['подкат']).strip() if pd.notna(row['подкат']) else '',
                'лист':     str(row['лист']).strip(),
                'строка':   str(row['строка']).strip(),
                'родитель': str(row['родитель']).strip() if pd.notna(row['родитель']) else '',
                'кол':      str(row['кол']).strip().upper() if pd.notna(row['кол']) else 'A',
            })
    return result
...
def write_to_target(target_path, results, mapping, month_num, year, log):
    ...
    for row in results:
        ...
        if key in val_lookup:
            log(f"⚠ Дубль ключа: завод='{key[0]}' подкат='{key[1]}' "
                f"старое={val_lookup[key]:.3f} → новое={row[4]:.3f}")
        val_lookup[key] = row[4]

    log(f"Уникальных ключей в данных: {len(val_lookup)}, строк маппинга: {len(mapping)}")
    ...
    for m in mapping:
        ...
        for col in ws.iter_cols(min_col=2, max_row=2):
            for cell in col:
                v = cell.value
                if isinstance(v, dt.datetime) and v.year == year and v.month == month_num:
        ...
        if target_row is None:
            not_found.append(f"{sheet_name}: строка '{row_label}' не найдена (родитель='{parent_label}')")
            continue
    ...
    try:
        ...
        raise PermissionError(f"Закройте файл в Excel и попробуйте снова:\n{target_path}")
...
def run_production(
    svod_folder, npk_file, tolyatti_folder,
    target_file, mapping_file,
    month_str, year,
    log, messagebox,
    stop_event,
):
    if not svod_folder:
        messagebox.showerror("Ошибка", "Укажите папку с файлами СВОД"); return
    if not target_file or not os.path.isfile(target_file):
        messagebox.showerror("Ошибка", "Укажите файл тестовые_данные.xlsx"); return
    if not mapping_file or not os.path.isfile(mapping_file):
        messagebox.showerror("Ошибка", "Укажите файл маппинга"); return

    filepath = find_svod_file(svod_folder, month_str)
    if not filepath:
        messagebox.showerror("Ошибка", f"Файл СВОД для месяца {month_str} не найден в:\n{svod_folder}"); return
    ...
    try:
        ...
        messagebox.showerror("Файл занят", str(e)); return
    except Exception as e:
