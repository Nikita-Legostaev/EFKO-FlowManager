...
def run_comparison(
    ...
    kuper = (
        ...
        .agg(
            Регулярная_Kuper=(
                "reg_kuper",
                lambda x: round(x.dropna().mean(), 2) if x.dropna().any() else None,
            ),
            Промо_Kuper=(
                "promo_kuper",
                lambda x: round(x.dropna().mean(), 2) if x.dropna().any() else None,
    ...
    data["metric"] = data[2].apply(
        lambda x: "regular" if "регулярная" in str(x) else "promo"
    )
    ...
    promo = (
        ...
        .pivot_table(
            index=["SKU", "Сеть"], columns="metric", values="last_val", aggfunc="first"
        )
        .reset_index()
    )
    promo.columns.name = None
    promo.rename(
        columns={"regular": "Регулярная_PromoData", "promo": "Промо_PromoData"},
        inplace=True,
    )
    ...
    promo_cnt = (
        ...
        .dropna(subset=["SKU", "Сеть"])
        .pivot_table(
            index=["SKU", "Сеть"], columns="metric", values="price_cnt", aggfunc="first"
        )
        .reset_index()
    )
    promo_cnt.columns.name = None
    promo_cnt.rename(
        columns={"regular": "N_рег_PD", "promo": "N_промо_PD"}, inplace=True
    )
    ...
    matched = (
        ...
        .merge(
            promo, left_on=["Сеть", "SKU_скорр"], right_on=["Сеть", "SKU"], how="inner"
        )
        .drop_duplicates(subset=["Сеть", "SKU_скорр"])
        .sort_values(["тип", "Сеть", "SKU_скорр"])
        .reset_index(drop=True)
    )
    matched["Δ Регулярная"] = (
        matched["Регулярная_Kuper"] - matched["Регулярная_PromoData"]
    ).round(2)
    matched["Δ Промо"] = (matched["Промо_Kuper"] - matched["Промо_PromoData"]).round(2)
    log(
        f"   Итоговых строк: {len(matched):,}  (Кетчуп: {len(matched[matched['тип']=='Кетчуп'])}, Майонез: {len(matched[matched['тип']=='Майонез'])})"
    )
...
def _write_excel(matched, output_file):
    ...
    ws["A1"] = (
        f"Купер vs PromoData  |  Майонез + Кетчуп томатный  |  "
        ...
        for ci, col in enumerate(DATA_COLS, 1):
            ...
            if ci in PRICE_COLS:
                ...
                if ci in (9, 10) and val is not None:
                    cell.fill = (
                        RED if float(val) > 0 else (GREEN if float(val) < 0 else fill)
                    )
                else:
                    cell.fill = fill
    ...
    for i, (txt, fnt) in enumerate(
        [
            ...
            (
                "  🔴 Красный Δ = Купер дороже PromoData",
                Font(name="Arial", size=9, color="CC0000"),
            ),
            (
                "  🟢 Зелёный Δ = Купер дешевле PromoData",
                Font(name="Arial", size=9, color="006600"),
            ),
            (
